"""W1-07 评测脚本真实数据冒烟测试（B 组 Baseline 版）。

用标注.zip 的 6 篇人工金标 vs DeepSeek B 组 Baseline（v2.0 提示词）的 9 篇 LLM 输出，
通过 project_identifier 匹配，跑 evaluate_dataset，输出评测报告到桌面。

B 组 Baseline 要求 LLM 输出 evidence_text，用于验证无依据输出率是否真实下降。
...
RECORDS_PATH = Path(r"C:\Users\Lenovo\Desktop\BidAgent\data\validation\baseline_real_b\records.jsonl")
# 输出目录
OUT_DIR = Path(r"C:\Users\Lenovo\Desktop\W1-07评测报告_B组")
