"""W3-07 数据库迁移：ExtractedField 表新增 display_grade + cross_verified 字段。
...
NEW_COLUMNS = [
    {
        "name": "display_grade",
        "sqlite_ddl": "VARCHAR(16) NOT NULL DEFAULT 'review'",
        "pg_ddl": "VARCHAR(16) NOT NULL DEFAULT 'review'",
    },
    {
        "name": "cross_verified",
        "sqlite_ddl": "BOOLEAN NOT NULL DEFAULT 0",
...
async def migrate_extracted_fields(
    ...
    *,
    ...
    Returns:
        {
            "table": bool,          # 表是否存在
            "migrated": [col_name], # 新增的列
            "skipped":  [col_name], # 已存在跳过的列
            "idempotent_run2": bool # 二次执行不报错
        }
    """
    if engine is None:
    ...
    for run_idx in range(2):
        async with engine.begin() as conn:
            ...
            if not has_table:
                # 表不存在则通过 metadata.create_all 已处理，无需 ALTER
                if run_idx == 0:
                    result["table"] = False
                else:
...
async def main() -> int:
    print("=" * 70)
    print("W3-07 迁移: extracted_fields 新增 display_grade + cross_verified")
    print("=" * 70)
    ...
    want = {"display_grade", "cross_verified"}
