...
标小智是一款面向中小企业和供应商的智能招投标信息聚合与字段抽取助手。针对招投标公告信息分散、字段抽取耗时、报价决策缺乏依据三大痛点，标小智构建了六 Agent 协作流水线：公告抓取 Agent（多平台轮询、HTTP 403 智能停止）、字段抽取 Agent（LLM + 证据定位双坐标映射）、证据验证 Agent（L1-L5 五级降级匹配、确定性校验）、风险预警 Agent（废标规则识别、供应商信用评分）、报告生成 Agent（Word 自动生成、附件管理）、推送调度 Agent（cron 定时、幂等推送）。
