...
// ========== 第 23 页：W2 证据验证闭环 ==========
{
  ...
  const rows = [
    ['fields_with_value', '31', '31', '29'],
    ['fields_with_evidence', '0', '31', '31'],
    ['unjustified_rate', '100%', '0%', '0%'],
    ['field_precision', '90.48%', '92.86%', '95.00%'],
    ['invalid_docs', '0', '0', '0'],
  ];
  ...
  // 结论
  s.addText('结论：A < B < C 分层显现，证据验证流水线有正向价值', {
  ...
  s.addText('recall 87.10%  ·  precision 74.36%  ·  iou_avg 0.6929  ·  iou_avg_matched 0.9242  ·  P50/P95=1.0', {
    x: 0.5, y: 6.0, w: 9, h: 0.4,
    fontFace: BODY_FONT, fontSize: 13, color: NAVY, align: 'left'
  });
  s.addText('金标：7 篇 × 6 字段 = 42 字段，模型 deepseek-v4-flash，max_tokens=8000', {
    x: 0.5, y: 6.5, w: 9, h: 0.3,
    fontFace: BODY_FONT, fontSize: 11, color: GREY, align: 'left', italic: true
  });
}
...
// ========== 第 26 页：安全合规 ==========
{
  ...
  const cols = [
    ...
    {
      title: 'AI 反幻觉',
      items: ['5 级降级匹配定位', '原文事实比对', '无依据字段不展示', 'IoU 0.92 边界质量', 'A<B<C 消融验证']
    },
  ];
  cols.forEach((c, i) => {
    const x = 0.5 + i * 3.15;
