...
为避免证据不完整或高亮错位，证据保存采用以下规则：

- **保存前预览弹窗**：显示选中文字、前后各 20 字符上下文、start/end、长度、slice 验证结果
- **向左/向右扩展1字**：方便补齐标签、标点和单位（如金额单位"万元"、企业角色"采购人"等）
- **重新选择**：关闭预览弹窗且不保存，等待用户重新选中文本
- **非阻塞质量提示**（不阻止保存，仅提示）：
  - 金额证据若没有"预算/限价/中标/成交/合同/单价"等类型词，提示证据可能不完整
  - 企业证据若没有"采购人/中标人/供应商/代理机构"等角色词，提示补充上下文
  - 日期证据若没有"发布/截止/开标"等类型词，提示补充上下文
